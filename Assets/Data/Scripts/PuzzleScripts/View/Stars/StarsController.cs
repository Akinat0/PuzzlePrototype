...
public class StarsController : MonoBehaviour
{
    ...
    public static StarsController Create(Transform parent)
    {
        StarsController starsController = new GameObject("stars_controller").AddComponent<StarsController>();
        starsController.transform.SetParent(parent);
        
        List<StarView> starViews = new List<StarView>();
        
        StarView starView = Resources.Load<StarView>("Prefabs/star_view");
        
        for (int i = 0; i < 3; i++)
            starViews.Add(Instantiate(starView, starsController.transform));
        
        starsController.Initialize(starViews);
    ...
    public void Initialize(IEnumerable<StarView> starViews)
    {
        ...
        float singleImageScale =
            ScreenScaler.FitHorizontalPart(StarsView.First().Background, 0.25f);
